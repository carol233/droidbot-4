...
class Device(object):
    ...
    def send_intent(self, intent):
        ...
        if isinstance(intent, Intent):
            ...
            cmd = intent
        return self.adb.shell(cmd.encode("utf-8"))
    ...
    def get_top_activity_name(self):
        ...
        #     # print m.group()
        #     return m.group(1)
        data = self.adb.shell("dumpsys activity top")
    ...
    def uninstall_app(self, app):
        ...
        adbshell = 'adb -s ' + self.serial + ' shell'
        log_cmd = "adb  -s " + self.serial + " pull " + "/data/data/de.robv.android.xposed.installer/log/error.log " + self.output_dir
        os.system(log_cmd)
        clear_cmd = [
            "> /data/data/de.robv.android.xposed.installer/log/error.log",
            "exit",
        ]
        clearlog = subprocess.Popen(adbshell, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
        time.sleep(3.5)
        clearlog.communicate("\n".join(clear_cmd) + "\n")
