    ...
    def generate_event_based_on_utg(self):
        ...
        for input_event in possible_events:
            if not self.utg.is_event_explored(event=input_event, state=current_state):
                def isAdView(var):
                    view_class = safe_dict_get(var, 'class')
                    if "WebView" in view_class:
                        return "Webview"
                    elif "ImageView" in view_class:
                        return "ImageView"
                    elif "ViewFlipper" in view_class:
                        return "ViewFlipper"
                    else:
                        return False
                def getFather(var, views):
                    view_parent = safe_dict_get(var, 'parent')
                    if view_parent == -1:
                        return False
                    else:
                        var = current_state.views[view_parent]
                        return var
                def isAdEvent(var, view_tree):
                    type = isAdView(var)
                    if type:
                        return type
                    else:
                        var = getFather(var, current_state.views)
                        if not var:
                            return False
                        else:
                            return isAdEvent(var, view_tree)

                if hasattr(input_event, "view") and hasattr(input_event, "event_type"):
                    if input_event.event_type == "touch":
                        var = input_event.view
                        view_tree = self.current_state.view_tree
                        type = isAdEvent(var, view_tree)
                        if type:
                            event = input_event.get_event_str(self.current_state)
                            size = safe_dict_get(var, 'size').split('*')
                                ...
                                if os.path.exists(cfg):
                                    with open(cfg, "r+") as f:
                                        ...
                                        for line in lines:
                                            if 'traffic=' in line:
                                                traffic = line.strip('\n').split('=')[-1]
                                                traffic_dir = traffic + ip + '/' + apkname
                                                sig = 0
                                                if not os.path.exists(traffic_dir):
                                                    os.mkdir(traffic_dir)
                                                else:
                                                    dirs = os.listdir(traffic_dir)
                                                    nums = []
                                                    for num in dirs:
                                                        if num.isdigit():
                                                            nums.append(int(num))
                                                    if len(nums):
                                                        ...
                                                        sig = 0
                                                with open(traffic_dir+"/clicked.txt", "a+") as fc:
                                                    fc.write("sig:" + str(sig) + " " +"type:"+type+' '+"detail:"+event + '\n')
                                return input_event
                            ...
                            if 'traffic=' in line:
                                ...
                                if not os.path.exists(traffic_dir):
                                    ...
                                    if len(nums):
                                        ...
                                        sig = 0
                                with open(traffic_dir + "/clicked.txt", "a+") as fc:
                                    fc.write("sig:" + str(sig) + " " + "type:no " + "detail:" + event  + '\n')
                return input_event
